...
export function PackingResultTable({ result, selectedPlacementId, onSelectPlacement }: PackingResultTableProps) {
  ...
  return <section className="packing-result-table" aria-label="Bảng chi tiết phương án xếp">
    ...
    <div className="result-table-scroll"><table><thead><tr><th>Thứ tự</th><th>Kiện hàng</th><th>Container</th><th>Vị trí (m)</th><th>Khối lượng</th></tr></thead><tbody>{visibleRows.map((row) => {
      ...
      return <tr className={selectedPlacementId === key ? 'selected' : ''} key={key}><td>{row.order}</td><td><button type="button" onClick={() => onSelectPlacement(key)}>{row.label}</button></td><td>{row.containerName}</td><td>{row.x.toFixed(1)} · {row.y.toFixed(1)} · {row.z.toFixed(1)}</td><td>{row.weight.toFixed(1)} kg</td></tr>;
