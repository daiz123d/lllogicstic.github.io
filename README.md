...
```text
172.16.4.12:5050/dai.ta/tesst:latest
```
...
GitLab CI uses Kaniko, so the runner does not need a local Docker daemon. Kubernetes nodes must allow the internal insecure registry `172.16.4.12:5050` if GitLab registry is served over HTTP.
